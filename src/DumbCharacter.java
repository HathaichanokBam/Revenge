...
public abstract class DumbCharacter extends Character{
	...
	/**
	public DumbCharacter[] getDCArray(int quantity, int yMin, int yMax, int x,int width, int height){
		DumbCharacter[] returnArray = new DumbCharacter[quantity];
		for(int i = 0; i < quantity; i++){
			
			returnArray[i] = new DumbCharacter(filename,extension,x, random.nextInt(yMax-yMin) + yMin, direction, frames, delay, width, height);
		}
		return returnArray;
	}
	*/
